...
import { DevelopersContext } from "../../store/developers";

function Profile(props) {
  console.log(props);
  const history = useHistory();
  const profile = props.profile;
  console.log(props.profile.skills);

  const DevelopersCtx = useContext(DevelopersContext);
  console.log(DevelopersCtx);

  const viewProfileHandler = () => {
    console.log("view profile");
    history.push(`/developers/${profile.handle}`);
    // fetch(`http://localhost:8080/api/profile/developers/${profile.handle}`)
    //   .then((res) => {
    //     if (res.ok) {
    //       console.log("res ok");
    //       res.json().then((data) => {
    //         console.log(data);
    //         DevelopersCtx.setIsProfile(data);
    //         history.push(`/developers/${profile.handle}`);
    //       });
    //     } else {
